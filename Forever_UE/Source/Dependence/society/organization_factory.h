...
// configuredArgs里(不并入registries,避免同一份参数存两份);CreateOrganization创建实例后
// 直接按id查configuredArgs、调用instance->ApplyArgs(),不需要mod自己关心参数从哪来。
class OrganizationFactory {
public:
	using CreateFunc = OrganizationMod*(*)();
	...
	// 阶段3占位:Mod导出的FinishModOrganizations(factory)按老约定会调用它,先留空实现。
