...
// "zone_mods"数组解析出的(id, 参数字符串)表交给Factory,一直保留在
// configuredArgs里(不并入registries,避免同一份参数存两份);CreateZone创建实例时
// 直接按id查configuredArgs、把参数字符串传给creator,mod自己的creator决定怎么用
// (传给构造函数/自己存着都行)——Factory不再替mod调用任何"创建后初始化"钩子。
class ZoneFactory {
public:
	using CreateFunc = ZoneMod*(*)(const std::string&);
...
private:
	...
	std::unordered_map<ZoneMod*, std::string> liveInstances;
