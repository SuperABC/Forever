...
- 31个具体子类：字段列表照抄老工程`event.h`，字段类型统一从`std::string`/`int`/`bool`改成
  `Expression`——呼应"理论上json里所有值都是表达式"的顶层原则（见`expression.md`），即使是像
  `PuzzleResultEvent::result`这种老工程是`int`的字段，这次也统一存成`Expression`，取值时调用方
  自己`EvaluateValue`+`ToInt`/`ToBool`/`ToString`按需转换。
- `OptionDialogEvent`不需要"按序号构造"这一种——老工程用参数类型重载区分`(int id, string
  option)`（按序号）和`(string name, string option)`（按名称）两种构造方式，这次确认按序号
  这种用法不需要，直接砍掉`id`字段，只保留`(Expression name, Expression option)`一种构造。
...
补一个`else if (type == "...")`分支，从JSON节点提取字段、用`Script::BuildExpression`包一层，
构造对应的`Event`子类。
