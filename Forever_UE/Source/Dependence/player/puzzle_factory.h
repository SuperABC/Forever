...
// "puzzle_mods"数组解析出的(id, 参数字符串)表交给Factory,一直保留在
// configuredArgs里(不并入registries,避免同一份参数存两份);CreatePuzzle创建实例后
// 直接按id查configuredArgs、调用instance->ApplyArgs(),不需要mod自己关心参数从哪来。
class PuzzleFactory {
public:
	using CreateFunc = PuzzleMod*(*)();
	using DestroyFunc = void(*)(PuzzleMod*);

	PuzzleFactory() = default;
	virtual ~PuzzleFactory() = default;

	virtual void RegisterPuzzle(const std::string& id, CreateFunc creator, DestroyFunc deleter);

	// 阶段3占位:Mod导出的FinishModPuzzles(factory)按老约定会调用它,先留空实现。
	virtual void CleanTemp();

	virtual PuzzleMod* CreatePuzzle(const std::string& id);

	// 必须走注册时mod提供的deleter释放,不能Factory直接delete——跨DLL new/delete安全,
	// 通过liveInstances反查实例对应的注册id、再取出对应deleter调用。
	virtual void DestroyPuzzle(PuzzleMod* instance);

	virtual bool CheckRegistered(const std::string& id) const;
	virtual std::vector<std::string> GetRegisteredIds() const;

	// 设置这个concept从config.json"puzzle_mods"数组解析出的(id, 参数字符串)表,
	// 必须在调用方触发mod的RegisterModPuzzles导出函数之前调用,否则CreatePuzzle
	// 拿实例创建时查不到对应参数。
...
private:
	...
	std::unordered_map<PuzzleMod*, std::string> liveInstances;
