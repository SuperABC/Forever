...
public:
	...
	virtual void DestroyApp(AppMod* instance);

	virtual bool CheckRegistered(const std::string& id) const;
	virtual std::vector<std::string> GetRegisteredIds() const;

	// 设置这个concept从config.json"app_mods"数组解析出的(id, 参数字符串)表,
	// 必须在调用方触发mod的RegisterModApps导出函数之前调用,否则CreateApp
	// 拿实例创建时查不到对应参数。
...
private:
	...
	std::unordered_map<AppMod*, std::string> liveInstances;
