...
`PostImplement`能拿到7个域的真实指针，不是提前实现Industry域——真正的业务聚合逻辑
（`Product`/`Storage`/`Manufacture`三个Mod扩展点已经在`Dependence`/`Basic`层铺好）留到
`PHASE4_PLAN.md`阶段4-5再点名做。
...
处理掉的Change转发给这六个域各自的`ApplyChange`——目前没有任何Change子类是Industry域
自己认识、需要处理的，`ApplyChange`因此是空实现，也**不打"未实现"警告**（只有
`Story::ApplyChange`保留那条兜底警告，避免六个域各打一遍重复日志）。等Industry域真正
