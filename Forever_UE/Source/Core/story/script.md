...
- **JSON解析只认`"milestones"`这一种数组结构**——老工程`ReadScript`同时支持"根节点是数组"和
  ...
  "路径->里程碑表"这一半。
- **`BuildEvent`/`BuildChanges`只识别`"game_start"`/`"set_value"`两种`type`**，其余识别到的
  `type`字符串统一`THROW_EXCEPTION(RuntimeException, ...)`，等对应类型被点名实现时再插入分支，
  见`Dependence/story/event.md`/`change.md`"JSON分发"一节。
...
- 其余69种event/change类型的JSON分发分支。
- `WrapScript`真正的改写逻辑（老工程的`actionStack`分层/`PlaceHolderChange`替换机制）——接口
  本身已经接通到`ScriptMod`，缺的是mod侧真正override它、往里塞自己长期持有的Dialog/Change。
