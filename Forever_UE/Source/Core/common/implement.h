...
//
// 这次只实现"random citizen"一种post类型（从Populace随机挑一个citizen姓名，供开局
// ChangeControlChange使用）。society/industry/traffic/player这四个指针这次构造出来传进去，
// 但Post()里还用不到，等对应域真正迁移出业务逻辑、需要通过Post查询它们的数据时再加新的
// else if分支。
