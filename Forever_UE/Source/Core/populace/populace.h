...
private:
	...
	// 实例——和Map::InitBuildings()等发现/注册mod dll的写法同一个模式，这次Populace自己
	// 独立持有一份ModLoader/NameFactory(不和Map共用，两者互不知道对方存在)。见
	...
	NameMod* nameMod = nullptr; // InitNames()创建，~Populace()里通过nameFactory.DestroyName()释放
