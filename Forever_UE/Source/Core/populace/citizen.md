...
身上的relatives/personality/acquaintances/assets/jobs/scheduler/educationExperiences等
字段——依赖还没迁移的Society/Industry/Job域，等对应域迁移到了再回来加，完整对照见
...
- 依赖：无（`<string>`外加`Lot`/`Zone`/`Building`/`Room`的前向声明）。
- 被谁依赖：`Source/Core/populace/populace.h/.cpp`（`Populace::GenerateCitizens()`
  `new Citizen(...)`，`~Populace()`析构）、`Source/Core/map/map.h/.cpp`
