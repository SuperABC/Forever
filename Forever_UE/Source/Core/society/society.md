...
拿到7个域的真实指针，不是提前实现Society域——真正的业务聚合逻辑（`Job`/`Calendar`/
`Organization`三个Mod扩展点已经在`Dependence`/`Basic`层铺好）留到`PHASE4_PLAN.md`阶段4-4
再点名做。

## 持有方式

和`Map`/`Populace`/`Story`一样，直接挂在`AForeverFrameworkActor`身上（`Society* society`
成员，`EnsureMapGenerated()`尾部`new`，`EndPlay`/析构函数里`delete`），不是被某个
