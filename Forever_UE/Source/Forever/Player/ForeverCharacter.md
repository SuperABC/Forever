...
- **T键切换控制的市民**(`SwitchControlledCitizen`)：定义在这个基类上，不是`ACitizenElement`
  自己的方法——不管当前被占有的是最初的`ADefaultPawn`/`AForeverCharacter`还是某个
  `ACitizenElement`，只要是`AForeverCharacter`的子类都自动获得同一份T键行为，不需要
  各自重复实现。逻辑很薄：`ACitizenElement::GetFirstNearby()`取"当前被占有对象附近的市民
  名单"（`ACitizenElement::nearbyCitizens`，由市民自己的碰撞盒`OnOverlapBegin`/
  `OnOverlapEnd`维护，详见`Element/CitizenElement.md`）里第一个仍然有效的市民，名单为空
  或者恰好就是自己（`target == this`，防止贴着自己的碰撞盒重复触发）时什么都不做，否则
  `GetController()->Possess(target)`——真正的移动模式切换（`MOVE_None`→`MOVE_Walking`）
  和Input Mapping Context的增删都在`ACitizenElement::PossessedBy`/`AForeverCharacter::
  PossessedBy`里跟着`Possess`自动发生，这个函数本身只负责"找到目标+发起换人"。绑定在
  `UForeverKeyBindingSubsystem`的`"Test"`动作（默认键`T`，见`Input/
  ForeverKeyBindingSubsystem.md`）上，`ETriggerEvent::Started`触发一次。
...
- 依赖`Source/Forever/Element/CitizenElement.h`（`SwitchControlledCitizen`调
  `ACitizenElement::GetFirstNearby()`）——这是`AForeverCharacter`唯一反向依赖自己子类
  的地方，`ACitizenElement`本身继承`AForeverCharacter`，见`Element/CitizenElement.md`。
