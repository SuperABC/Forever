...
为`Source/Dependence`21个`<Concept>Mod`接口各配一个`<Concept>Basic`默认实现占位,和
`Source/Dependence/README.md`的21个concept对照表一一对应(`map/terrain_basic.h`→
...
的机制:根目录`basic.cpp`用`extern "C" __declspec(dllexport)`导出21组
