...
void BuildingBasic::Layout(int& direction, const Quad& quad,
	...
	if (layout == 0) {
		...
		ArrangeRow(-1, 0, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(-1, 1, RoomBasic::GetId(), size, kComponent, kComponentId);
		AssignFloor(0, "preset_straight_linear_fg+-", direction);
		ArrangeRow(0, 0, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(0, 1, RoomBasic::GetId(), size, kComponent, kComponentId);
		for (int i = 1; i < layers; i++) {
			AssignFloor(i, "preset_straight_linear_fu+-", direction);
			ArrangeRow(i, 0, RoomBasic::GetId(), size, kComponent, kComponentId);
			ArrangeRow(i, 1, RoomBasic::GetId(), size, kComponent, kComponentId);
		}
	} else if (layout == 1) {
		AssignFloor(-1, "preset_lobby_wing_bg+", direction);
		ArrangeRow(-1, 0, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(-1, 1, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(-1, 2, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(-1, 3, RoomBasic::GetId(), size, kComponent, kComponentId);
		AssignFloor(0, "preset_lobby_wing_fg+-", direction);
		ArrangeRow(0, 0, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(0, 1, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(0, 2, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(0, 3, RoomBasic::GetId(), size, kComponent, kComponentId);
		for (int i = 1; i < layers; i++) {
			AssignFloor(i, "preset_lobby_wing_fu+-", direction);
			ArrangeRow(i, 0, RoomBasic::GetId(), size, kComponent, kComponentId);
			ArrangeRow(i, 1, RoomBasic::GetId(), size, kComponent, kComponentId);
			ArrangeRow(i, 2, RoomBasic::GetId(), size, kComponent, kComponentId);
			ArrangeRow(i, 3, RoomBasic::GetId(), size, kComponent, kComponentId);
		}
		...
		AssignRoom(-1, 0, RoomBasic::GetId(), kComponent, kComponentId);
		AssignRoom(-1, 1, RoomBasic::GetId(), kComponent, kComponentId);
		ArrangeRow(-1, 0, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(-1, 1, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(-1, 2, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(-1, 3, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(-1, 4, RoomBasic::GetId(), size, kComponent, kComponentId);
		AssignFloor(0, "preset_lshape_double_fg+-", direction);
		AssignRoom(0, 0, RoomBasic::GetId(), kComponent, kComponentId);
		AssignRoom(0, 1, RoomBasic::GetId(), kComponent, kComponentId);
		ArrangeRow(0, 0, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(0, 1, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(0, 2, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(0, 3, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(0, 4, RoomBasic::GetId(), size, kComponent, kComponentId);
		for (int i = 1; i < layers; i++) {
			AssignFloor(i, "preset_lshape_double_fu+-", direction);
			AssignRoom(i, 0, RoomBasic::GetId(), kComponent, kComponentId);
			AssignRoom(i, 1, RoomBasic::GetId(), kComponent, kComponentId);
			ArrangeRow(i, 0, RoomBasic::GetId(), size, kComponent, kComponentId);
			ArrangeRow(i, 1, RoomBasic::GetId(), size, kComponent, kComponentId);
			ArrangeRow(i, 2, RoomBasic::GetId(), size, kComponent, kComponentId);
			ArrangeRow(i, 3, RoomBasic::GetId(), size, kComponent, kComponentId);
			ArrangeRow(i, 4, RoomBasic::GetId(), size, kComponent, kComponentId);
		}
	} else { // layout == 3
		AssignFloor(-1, "preset_nshape_double_bg+", direction);
		AssignRoom(-1, 0, "empty", kComponent, kComponentId); // 老工程这里也是"empty"占位槽位
		AssignRoom(-1, 1, RoomBasic::GetId(), kComponent, kComponentId);
		AssignRoom(-1, 2, RoomBasic::GetId(), kComponent, kComponentId);
		ArrangeRow(-1, 0, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(-1, 1, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(-1, 2, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(-1, 3, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(-1, 4, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(-1, 5, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(-1, 6, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(-1, 7, RoomBasic::GetId(), size, kComponent, kComponentId);
		AssignFloor(0, "preset_nshape_double_fg+-", direction);
		AssignRoom(0, 0, "empty", kComponent, kComponentId);
		AssignRoom(0, 1, RoomBasic::GetId(), kComponent, kComponentId);
		AssignRoom(0, 2, RoomBasic::GetId(), kComponent, kComponentId);
		ArrangeRow(0, 0, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(0, 1, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(0, 2, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(0, 3, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(0, 4, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(0, 5, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(0, 6, RoomBasic::GetId(), size, kComponent, kComponentId);
		ArrangeRow(0, 7, RoomBasic::GetId(), size, kComponent, kComponentId);
		for (int i = 1; i < layers; i++) {
			AssignFloor(i, "preset_nshape_double_fu+-", direction);
			AssignRoom(i, 0, "empty", kComponent, kComponentId);
			AssignRoom(i, 1, RoomBasic::GetId(), kComponent, kComponentId);
			AssignRoom(i, 2, RoomBasic::GetId(), kComponent, kComponentId);
			ArrangeRow(i, 0, RoomBasic::GetId(), size, kComponent, kComponentId);
			ArrangeRow(i, 1, RoomBasic::GetId(), size, kComponent, kComponentId);
			ArrangeRow(i, 2, RoomBasic::GetId(), size, kComponent, kComponentId);
			ArrangeRow(i, 3, RoomBasic::GetId(), size, kComponent, kComponentId);
			ArrangeRow(i, 4, RoomBasic::GetId(), size, kComponent, kComponentId);
			ArrangeRow(i, 5, RoomBasic::GetId(), size, kComponent, kComponentId);
			ArrangeRow(i, 6, RoomBasic::GetId(), size, kComponent, kComponentId);
			ArrangeRow(i, 7, RoomBasic::GetId(), size, kComponent, kComponentId);
		}
		AssignElevatorCabin(0, -basements, layers - 1, "");
		AssignElevatorCabin(1, -basements, layers - 1, "");
	}
